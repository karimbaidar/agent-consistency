...
The benchmark runs paired raw and protected modes for each deterministic
scenario. Raw mode represents a naive agent that reports success while the
business outcome is false. Protected mode routes the same situation through
agent-consistency gates and records whether the false success was caught.

Benchmark results are generated by `python -m benchmark.run --write-results
benchmark/results.md` and should be described as scenario-suite results, not as
